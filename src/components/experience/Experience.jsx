  ...
  return (
    <section className="experience" id="experience">
      ...
      <div data-target="load-content">
        {
          Util.getUniqueElements(experiences.filter(e => e.visible && e.active), 'name').toSorted((a, b) => a.difficult > b.difficult).reverse().map((project, i) => {
            return (
              <div key={i} className="experience__box" data-aos="fade-up">
                <img className="experience__midia expand hover-scale" loading="lazy"
                     src={project.img || 'https://via.placeholder.com/600x400.png?text=Imagem...'}
                     alt={`Captura de tela do projeto ${project.name.trim()}`}
                     style={project.img.includes('https://opengraph.githubassets.com') ? {aspectRatio: "initial"} : {}}
                     onClick={(event) => {
                       event.preventScroll = true;
                       expandImageInfos.setSrc(project.img);
                       expandImageInfos.setAlt(`Captura de tela do projeto ${project.name.trim()}`);
                ...
                <div className="experience__info">
                  ...
                  <div className="experience__description">
                    {project.link && project.link.length > 0 && project.link !== "#" ?
                      <span className="experience__repo"><a href={project.link.trim()} target="_blank"><button
                        className="experiencia__botao--repo">Repositório</button></a></span> : ''}
                    {project.demo && project.demo.length > 0 && project.demo !== '#' ?
                      <span className="experience__demo"><a href={project.demo.trim()} target="_blank"><button
                        className="experiencia__botao--demo">Ver demo</button></a></span> : ''}
                  </div>
                </div>
              </div>
            )
